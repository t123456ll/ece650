...
def string_to_number(coordinate):  #the coordinate in dictionary is string
    ...
    row_num = int(len(templist)/2)
    list=[[0 for col in range(3)]for row in range(row_num)]
...
def findDel(list_edges,line_find):
    for nn in range(len(list_edges)):
        if list_edges[nn] == line_find:
            del list_edges[nn]
            return 1
    return 0
...
def printVE(v,edges):
    ...
    sys.stdout.write("V " + str(v_num + 1) + "\n" + "E {")
    sys.stdout.flush()
...
def main():
    ...
    while True:
        ...
        res1 = re.search(r'(?P<processing>[arRg]{1}).*',linestr)
        #res1 = re.search(r'(?P<processing>[acrg]{1})\W{2,}[\w+]*\s[\w+]*\W{2,}',linestr)
        try:
            tempstreetdic = res1.groupdict()
        except AttributeError:
            sys.stderr.write( "Error: wrong command 1\n")
            continue
        
        if tempstreetdic['processing']=='g':
            namelist=list(streetdic.keys())
            valuelist=[]
            if namelist:
                streetdic[namelist[0]][0][2]=0
                ...
                while (flag == 1):
                    flag = 0
                    len_edges = int(len(edges))

                    for ii in range(position_ii, len_edges):
                        for jj in range(position_jj, len_edges):
                            ...
                            if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                                if (pp1 in [pp3, pp4]):
                                    if (onSegment(pp3.x, pp3.y, pp4.x, pp4.y, pp2.x, pp2.y)):
                                        ...
                                        deledge1 = edges[ii]
                                        deledge2 = edges[jj]
                                        findDel(edges, deledge1)
                                        findDel(edges, deledge2)
            ...
            res1 = re.search(r'(?P<processing>[arRg]{1})\s+\"{1}(?P<name>\s*[A-Za-z+]*\s*[A-Za-z+]*\s*[A-Za-z+]*\s*[A-Za-z+]*)\"',linestr)
            try:
                tempstreetdic = res1.groupdict()
            except AttributeError:
                sys.stderr.write( "Error: wrong command 2\n")
                continue
            
            if tempstreetdic['processing']=='a':
                res1 = re.search(r'(?P<processing>[arRg]{1})\s+\"{1}(?P<name>\s*[A-Za-z+]*\s*[A-Za-z+]*\s*[A-Za-z+]*\s*[A-Za-z+]*)\W{2,}(?P<coordinate>\(.{8,}\))',linestr)
                tempstreetdic = res1.groupdict()
                streetdic[str(tempstreetdic['name']).lower()]=string_to_number(tempstreetdic['coordinate'])
                
            elif tempstreetdic['processing']=='r':
                delstreet = str(tempstreetdic['name']).lower();
                #if streetdic.has_key(delstreet):
                try:
                    del streetdic[ delstreet ];
                except KeyError:
                    continue
                ...
                sys.stderr.write( "Error: wrong command 3\n" )
...
"""
            ...
            for i in range(2):
                final_edges = edges[:]
                len_final_edges = int(len(final_edges))
                len_edges = int(len(edges))
                for ii in range(len_edges):
                    for jj in range(ii+1, len_edges):
                        ...
                        if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                            if (pp1 in [pp3, pp4]):
                                if (onSegment(pp3.x, pp3.y, pp4.x, pp4.y, pp2.x, pp2.y)):
                                    ...
                                    findDel(final_edges, edges[ii])
                                    findDel(final_edges, edges[jj])

                                    if (line(label(v, pp2), label(v, pp3)) not in final_edges):
                                        if (label(v, pp2) != label(v, pp3)):
                                            final_edges.append(line(label(v, pp2), label(v, pp3)))
                                    if (line(label(v, pp2), label(v, pp4)) not in final_edges):
                                        if (label(v, pp2) != label(v, pp4)):
                                            final_edges.append(line(label(v, pp2), label(v, pp4)))
                        ...
                        if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                            if (pp2 in [pp3, pp4]):
                                if (onSegment(pp3.x, pp3.y, pp4.x, pp4.y, pp1.x, pp1.y)):
                                    ...
                                    if (line(label(v, pp1), label(v, pp3)) not in final_edges):
                                        if (label(v, pp1) != label(v, pp3)):
                                            final_edges.append(line(label(v, pp1), label(v, pp3)))
                                    if (line(label(v, pp1), label(v, pp4)) not in final_edges):
                                        if (label(v, pp1) != label(v, pp4)):
                                            final_edges.append(line(label(v, pp1), label(v, pp4)))
                        ...
                        if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                            if (pp3 in [pp1, pp2]):
                                if (onSegment(pp1.x, pp1.y, pp2.x, pp2.y, pp4.x, pp4.y)):
                                    ...
                                    if (line(label(v, pp4), label(v, pp1)) not in final_edges):
                                        if (label(v, pp4) != label(v, pp1)):
                                            final_edges.append(line(label(v, pp4), label(v, pp1)))
                                    if (line(label(v, pp4), label(v, pp2)) not in final_edges):
                                        if (label(v, pp4) != label(v, pp2)):
                                            final_edges.append(line(label(v, pp4), label(v, pp2)))
                        ...
                        if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                            if (pp4 in [pp1, pp2]):
                                if (onSegment(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y)):
                                    ...
                                    if (line(label(v, pp3), label(v, pp1)) not in final_edges):
                                        if (label(v, pp3) != label(v, pp1)):
                                            final_edges.append(line(label(v, pp3), label(v, pp1)))
                                    if (line(label(v, pp3), label(v, pp2)) not in final_edges):
                                        if (label(v, pp3) != label(v, pp2)):
                                            final_edges.append(line(label(v, pp3), label(v, pp2)))
                        ...
                        if (onSegment1(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                            if (parallel(pp1.x, pp1.y, pp2.x, pp2.y, pp3.x, pp3.y, pp4.x, pp4.y)):
                                ...
                                if (line(label(v, cur_edges2[0]), label(v, cur_edges2[1])) not in final_edges):
                                    if (label(v, cur_edges2[0]) != label(v, cur_edges2[1])):
                                        final_edges.append(line(label(v, cur_edges2[0]), label(v, cur_edges2[1])))
                                if (line(label(v, cur_edges2[1]), label(v, cur_edges2[2])) not in final_edges):
                                    if (label(v, cur_edges2[1]) != label(v, cur_edges2[2])):
                                        final_edges.append(line(label(v, cur_edges2[1]), label(v, cur_edges2[2])))
                                if (line(label(v, cur_edges2[2]), label(v, cur_edges2[3])) not in final_edges):
                                    if (label(v, cur_edges2[2]) != label(v, cur_edges2[3])):
                                        final_edges.append(line(label(v, cur_edges2[2]), label(v, cur_edges2[3])))
